...
saga_rasterorientation <- function(INPUT = qgisprocess:::qgis_default_value(), METHOD = qgisprocess:::qgis_default_value(), RESULT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("saga:rasterorientation")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("saga:rasterorientation", `INPUT` = INPUT, `METHOD` = METHOD, `RESULT` = RESULT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("saga:rasterorientation", `INPUT` = INPUT, `METHOD` = METHOD, `RESULT` = RESULT,..., .quiet = .quiet)
      )
