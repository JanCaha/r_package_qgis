...
##' @param VALUES `string` - List of target pixels. String value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
...
##' @param MAX_DISTANCE `number` - The maximum distance to generate. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param REPLACE `number` - Value to apply to pixels within the maximum distance of target pixels. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
...
gdal_proximity <- function(INPUT = qgisprocess:::qgis_default_value(), BAND = qgisprocess:::qgis_default_value(), VALUES = qgisprocess:::qgis_default_value(), UNITS = qgisprocess:::qgis_default_value(), MAX_DISTANCE = qgisprocess:::qgis_default_value(), REPLACE = qgisprocess:::qgis_default_value(), NODATA = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), DATA_TYPE = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("gdal:proximity")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:proximity", `INPUT` = INPUT, `BAND` = BAND, `VALUES` = VALUES, `UNITS` = UNITS, `MAX_DISTANCE` = MAX_DISTANCE, `REPLACE` = REPLACE, `NODATA` = NODATA, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:proximity", `INPUT` = INPUT, `BAND` = BAND, `VALUES` = VALUES, `UNITS` = UNITS, `MAX_DISTANCE` = MAX_DISTANCE, `REPLACE` = REPLACE, `NODATA` = NODATA, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
