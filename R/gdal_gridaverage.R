...
##' @param OUTPUT `rasterDestination` - Interpolated (moving average). Path for new raster layer.
...
##' @details
##' ## Outputs description
##' * OUTPUT - outputRaster - Interpolated (moving average)
##'
##'
##' @export
##' @md
...
gdal_gridaverage <- function(INPUT = qgisprocess:::qgis_default_value(), Z_FIELD = qgisprocess:::qgis_default_value(), RADIUS_1 = qgisprocess:::qgis_default_value(), RADIUS_2 = qgisprocess:::qgis_default_value(), ANGLE = qgisprocess:::qgis_default_value(), MIN_POINTS = qgisprocess:::qgis_default_value(), NODATA = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), DATA_TYPE = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("gdal:gridaverage")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:gridaverage", `INPUT` = INPUT, `Z_FIELD` = Z_FIELD, `RADIUS_1` = RADIUS_1, `RADIUS_2` = RADIUS_2, `ANGLE` = ANGLE, `MIN_POINTS` = MIN_POINTS, `NODATA` = NODATA, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:gridaverage", `INPUT` = INPUT, `Z_FIELD` = Z_FIELD, `RADIUS_1` = RADIUS_1, `RADIUS_2` = RADIUS_2, `ANGLE` = ANGLE, `MIN_POINTS` = MIN_POINTS, `NODATA` = NODATA, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
