##' QGIS Algorithm provided by GRASS m.cogo (grass7:m.cogo). A simple utility for converting bearing and distance measurements to coordinates and vice versa. It assumes a Cartesian coordinate system
##'
...
grass7_m_cogo <- function(input = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), coordinates = qgisprocess:::qgis_default_value(), .l = qgisprocess:::qgis_default_value(), .q = qgisprocess:::qgis_default_value(), .r = qgisprocess:::qgis_default_value(), .c = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {
  ...
  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass7:m.cogo", `input` = input, `output` = output, `coordinates` = coordinates, `-l` = .l, `-q` = .q, `-r` = .r, `-c` = .c, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass7:m.cogo", `input` = input, `output` = output, `coordinates` = coordinates, `-l` = .l, `-q` = .q, `-r` = .r, `-c` = .c, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER,..., .quiet = .quiet)
      )
