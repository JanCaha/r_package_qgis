##' QGIS Algorithm provided by QGIS (native c++) Raster calculator (native:modelerrastercalc). Performing algebraic operations using raster layers.
##'
...
qgis_modelerrastercalc <- function(LAYERS = qgisprocess:::qgis_default_value(), EXPRESSION = qgisprocess:::qgis_default_value(), EXTENT = qgisprocess:::qgis_default_value(), CELL_SIZE = qgisprocess:::qgis_default_value(), CRS = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:modelerrastercalc")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:modelerrastercalc", `LAYERS` = LAYERS, `EXPRESSION` = EXPRESSION, `EXTENT` = EXTENT, `CELL_SIZE` = CELL_SIZE, `CRS` = CRS, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:modelerrastercalc", `LAYERS` = LAYERS, `EXPRESSION` = EXPRESSION, `EXTENT` = EXTENT, `CELL_SIZE` = CELL_SIZE, `CRS` = CRS, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
