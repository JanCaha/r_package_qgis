...
sagang_addrastervaluestofeatures <- function(SHAPES = qgisprocess:::qgis_default_value(), GRIDS = qgisprocess:::qgis_default_value(), RESULT = qgisprocess:::qgis_default_value(), RESAMPLING = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:addrastervaluestofeatures")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:addrastervaluestofeatures", `SHAPES` = SHAPES, `GRIDS` = GRIDS, `RESULT` = RESULT, `RESAMPLING` = RESAMPLING,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:addrastervaluestofeatures", `SHAPES` = SHAPES, `GRIDS` = GRIDS, `RESULT` = RESULT, `RESAMPLING` = RESAMPLING,..., .quiet = .quiet)
      )
