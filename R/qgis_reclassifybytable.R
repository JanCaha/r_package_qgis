##' QGIS Algorithm provided by QGIS (native c++) Reclassify by table (native:reclassifybytable). Reclassifies a raster band by assigning new class values based on the ranges specified in a fixed table. This algorithm reclassifies a raster band by assigning new class values based on the ranges specified in a fixed table.
##'
...
qgis_reclassifybytable <- function(INPUT_RASTER = qgisprocess:::qgis_default_value(), RASTER_BAND = qgisprocess:::qgis_default_value(), TABLE = qgisprocess:::qgis_default_value(), NO_DATA = qgisprocess:::qgis_default_value(), RANGE_BOUNDARIES = qgisprocess:::qgis_default_value(), NODATA_FOR_MISSING = qgisprocess:::qgis_default_value(), DATA_TYPE = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:reclassifybytable")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:reclassifybytable", `INPUT_RASTER` = INPUT_RASTER, `RASTER_BAND` = RASTER_BAND, `TABLE` = TABLE, `NO_DATA` = NO_DATA, `RANGE_BOUNDARIES` = RANGE_BOUNDARIES, `NODATA_FOR_MISSING` = NODATA_FOR_MISSING, `DATA_TYPE` = DATA_TYPE, `CREATION_OPTIONS` = CREATION_OPTIONS, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:reclassifybytable", `INPUT_RASTER` = INPUT_RASTER, `RASTER_BAND` = RASTER_BAND, `TABLE` = TABLE, `NO_DATA` = NO_DATA, `RANGE_BOUNDARIES` = RANGE_BOUNDARIES, `NODATA_FOR_MISSING` = NODATA_FOR_MISSING, `DATA_TYPE` = DATA_TYPE, `CREATION_OPTIONS` = CREATION_OPTIONS, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
