...
sagang_transectthroughpolygonshapefile <- function(TRANSECT = qgisprocess:::qgis_default_value(), THEME = qgisprocess:::qgis_default_value(), THEME_FIELD = qgisprocess:::qgis_default_value(), TRANSECT_RESULT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:transectthroughpolygonshapefile")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:transectthroughpolygonshapefile", `TRANSECT` = TRANSECT, `THEME` = THEME, `THEME_FIELD` = THEME_FIELD, `TRANSECT_RESULT` = TRANSECT_RESULT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:transectthroughpolygonshapefile", `TRANSECT` = TRANSECT, `THEME` = THEME, `THEME_FIELD` = THEME_FIELD, `TRANSECT_RESULT` = TRANSECT_RESULT,..., .quiet = .quiet)
      )
