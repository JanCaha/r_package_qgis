##' QGIS Algorithm provided by GRASS i.gensig (grass7:i.gensig). Generates statistics for i.maxlik from raster map.
##'
...
grass7_i_gensig <- function(trainingmap = qgisprocess:::qgis_default_value(), input = qgisprocess:::qgis_default_value(), signaturefile = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {
  ...
  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass7:i.gensig", `trainingmap` = trainingmap, `input` = input, `signaturefile` = signaturefile, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass7:i.gensig", `trainingmap` = trainingmap, `input` = input, `signaturefile` = signaturefile, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
