##' QGIS Algorithm provided by GRASS v.extract (grass:v.extract). Selects vector objects from a vector layer and creates a new layer containing only the selected objects.
##'
...
##' @param file `file` - Input text file with category numbers\code{/}number ranges to be extracted. Path to a file.
##' @param random `number` - Number of random categories matching vector objects to extract. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param new `number` - Desired new category value (enter -1 to keep original categories). A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param .d `boolean` - Dissolve common boundaries. 1 for true/yes. 0 for false/no. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression. Original algorithm parameter name: -d.
