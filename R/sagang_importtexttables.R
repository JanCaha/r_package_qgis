...
##'
##' @title QGIS algorithm - Import text tables
##'
...
sagang_importtexttables <- function(TABLES = qgisprocess:::qgis_default_value(), FILES = qgisprocess:::qgis_default_value(), HEADLINE = qgisprocess:::qgis_default_value(), SEPARATOR = qgisprocess:::qgis_default_value(), SEP_OTHER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:importtexttables")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:importtexttables", `TABLES` = TABLES, `FILES` = FILES, `HEADLINE` = HEADLINE, `SEPARATOR` = SEPARATOR, `SEP_OTHER` = SEP_OTHER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:importtexttables", `TABLES` = TABLES, `FILES` = FILES, `HEADLINE` = HEADLINE, `SEPARATOR` = SEPARATOR, `SEP_OTHER` = SEP_OTHER,..., .quiet = .quiet)
      )
