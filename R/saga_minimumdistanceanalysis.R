...
saga_minimumdistanceanalysis <- function(POINTS = qgisprocess:::qgis_default_value(), TABLE = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("saga:minimumdistanceanalysis")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("saga:minimumdistanceanalysis", `POINTS` = POINTS, `TABLE` = TABLE,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("saga:minimumdistanceanalysis", `POINTS` = POINTS, `TABLE` = TABLE,..., .quiet = .quiet)
      )
