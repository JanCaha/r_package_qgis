##' QGIS Algorithm provided by GRASS r.stats.quantile.rast (grass:r.stats.quantile.rast). r.stats.quantile.rast - Compute category quantiles using two passes and output rasters.
##'
...
grass_r_stats_quantile_rast <- function(base = qgisprocess:::qgis_default_value(), cover = qgisprocess:::qgis_default_value(), quantiles = qgisprocess:::qgis_default_value(), percentiles = qgisprocess:::qgis_default_value(), bins = qgisprocess:::qgis_default_value(), .r = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:r.stats.quantile.rast")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:r.stats.quantile.rast", `base` = base, `cover` = cover, `quantiles` = quantiles, `percentiles` = percentiles, `bins` = bins, `-r` = .r, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:r.stats.quantile.rast", `base` = base, `cover` = cover, `quantiles` = quantiles, `percentiles` = percentiles, `bins` = bins, `-r` = .r, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
