...
##'
##' @title QGIS algorithm - Conditional branch
##'
...
qgis_condition <- function(..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:condition")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:condition", ,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:condition", ,..., .quiet = .quiet)
      )
