##' QGIS Algorithm provided by GRASS v.build.polylines (grass7:v.build.polylines). Builds polylines from lines or boundaries.
##'
##' @title QGIS algorithm - v.build.polylines
##'
##' @param input `source` - Name of input vector map. Path to a vector layer.
##' @param cats `enum`  of `("no", "first", "multi", "same")` - Category number mode. Number of selected option, e.g. '1'. Comma separated list of options, e.g. '1,3'.
##' @param type `enum`  of `("line", "boundary")` - Input feature type. Number of selected option, e.g. '1'. Comma separated list of options, e.g. '1,3'.
##' @param output `vectorDestination` - Polylines. Path for new vector layer.
