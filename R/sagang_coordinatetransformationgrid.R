...
##' @param SOURCE `raster` - Source. Path to a raster layer.
...
##' @param GRID `rasterDestination` - Target. Path for new raster layer.
...
sagang_coordinatetransformationgrid <- function(CRS_METHOD = qgisprocess:::qgis_default_value(), CRS_PROJ4 = qgisprocess:::qgis_default_value(), CRS_FILE = qgisprocess:::qgis_default_value(), CRS_EPSG = qgisprocess:::qgis_default_value(), CRS_EPSG_AUTH = qgisprocess:::qgis_default_value(), SOURCE = qgisprocess:::qgis_default_value(), RESAMPLING = qgisprocess:::qgis_default_value(), BYTEWISE = qgisprocess:::qgis_default_value(), KEEP_TYPE = qgisprocess:::qgis_default_value(), TARGET_AREA = qgisprocess:::qgis_default_value(), TARGET_USER_XMIN_TARGET_USER_XMAX_TARGET_USER_YMIN_TARGET_USER_YMAX = qgisprocess:::qgis_default_value(), TARGET_USER_SIZE = qgisprocess:::qgis_default_value(), GRID = qgisprocess:::qgis_default_value(), OUT_X = qgisprocess:::qgis_default_value(), OUT_Y = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:coordinatetransformationgrid")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:coordinatetransformationgrid", `CRS_METHOD` = CRS_METHOD, `CRS_PROJ4` = CRS_PROJ4, `CRS_FILE` = CRS_FILE, `CRS_EPSG` = CRS_EPSG, `CRS_EPSG_AUTH` = CRS_EPSG_AUTH, `SOURCE` = SOURCE, `RESAMPLING` = RESAMPLING, `BYTEWISE` = BYTEWISE, `KEEP_TYPE` = KEEP_TYPE, `TARGET_AREA` = TARGET_AREA, `TARGET_USER_XMIN TARGET_USER_XMAX TARGET_USER_YMIN TARGET_USER_YMAX` = TARGET_USER_XMIN_TARGET_USER_XMAX_TARGET_USER_YMIN_TARGET_USER_YMAX, `TARGET_USER_SIZE` = TARGET_USER_SIZE, `GRID` = GRID, `OUT_X` = OUT_X, `OUT_Y` = OUT_Y,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:coordinatetransformationgrid", `CRS_METHOD` = CRS_METHOD, `CRS_PROJ4` = CRS_PROJ4, `CRS_FILE` = CRS_FILE, `CRS_EPSG` = CRS_EPSG, `CRS_EPSG_AUTH` = CRS_EPSG_AUTH, `SOURCE` = SOURCE, `RESAMPLING` = RESAMPLING, `BYTEWISE` = BYTEWISE, `KEEP_TYPE` = KEEP_TYPE, `TARGET_AREA` = TARGET_AREA, `TARGET_USER_XMIN TARGET_USER_XMAX TARGET_USER_YMIN TARGET_USER_YMAX` = TARGET_USER_XMIN_TARGET_USER_XMAX_TARGET_USER_YMIN_TARGET_USER_YMAX, `TARGET_USER_SIZE` = TARGET_USER_SIZE, `GRID` = GRID, `OUT_X` = OUT_X, `OUT_Y` = OUT_Y,..., .quiet = .quiet)
      )
