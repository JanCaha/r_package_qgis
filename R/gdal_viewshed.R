...
gdal_viewshed <- function(INPUT = qgisprocess:::qgis_default_value(), BAND = qgisprocess:::qgis_default_value(), OBSERVER = qgisprocess:::qgis_default_value(), OBSERVER_HEIGHT = qgisprocess:::qgis_default_value(), TARGET_HEIGHT = qgisprocess:::qgis_default_value(), MAX_DISTANCE = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("gdal:viewshed")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:viewshed", `INPUT` = INPUT, `BAND` = BAND, `OBSERVER` = OBSERVER, `OBSERVER_HEIGHT` = OBSERVER_HEIGHT, `TARGET_HEIGHT` = TARGET_HEIGHT, `MAX_DISTANCE` = MAX_DISTANCE, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:viewshed", `INPUT` = INPUT, `BAND` = BAND, `OBSERVER` = OBSERVER, `OBSERVER_HEIGHT` = OBSERVER_HEIGHT, `TARGET_HEIGHT` = TARGET_HEIGHT, `MAX_DISTANCE` = MAX_DISTANCE, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
