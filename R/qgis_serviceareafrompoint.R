##' QGIS Algorithm provided by QGIS (native c++) Service area (from point) (native:serviceareafrompoint)
##'
...
##' @param TRAVEL_COST2 `number` - Travel cost (distance for 'Shortest', time for 'Fastest'). A numeric value.
##' @param INCLUDE_BOUNDS `boolean` - Include upper/lower bound points. 1 for true/yes. 0 for false/no.
...
qgis_serviceareafrompoint <- function(INPUT = qgisprocess::qgis_default_value(), STRATEGY = qgisprocess::qgis_default_value(), DIRECTION_FIELD = qgisprocess::qgis_default_value(), VALUE_FORWARD = qgisprocess::qgis_default_value(), VALUE_BACKWARD = qgisprocess::qgis_default_value(), VALUE_BOTH = qgisprocess::qgis_default_value(), DEFAULT_DIRECTION = qgisprocess::qgis_default_value(), SPEED_FIELD = qgisprocess::qgis_default_value(), DEFAULT_SPEED = qgisprocess::qgis_default_value(), TOLERANCE = qgisprocess::qgis_default_value(), START_POINT = qgisprocess::qgis_default_value(), TRAVEL_COST2 = qgisprocess::qgis_default_value(), INCLUDE_BOUNDS = qgisprocess::qgis_default_value(), OUTPUT_LINES = qgisprocess::qgis_default_value(), OUTPUT = qgisprocess::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:serviceareafrompoint")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:serviceareafrompoint", `INPUT` = INPUT, `STRATEGY` = STRATEGY, `DIRECTION_FIELD` = DIRECTION_FIELD, `VALUE_FORWARD` = VALUE_FORWARD, `VALUE_BACKWARD` = VALUE_BACKWARD, `VALUE_BOTH` = VALUE_BOTH, `DEFAULT_DIRECTION` = DEFAULT_DIRECTION, `SPEED_FIELD` = SPEED_FIELD, `DEFAULT_SPEED` = DEFAULT_SPEED, `TOLERANCE` = TOLERANCE, `START_POINT` = START_POINT, `TRAVEL_COST2` = TRAVEL_COST2, `INCLUDE_BOUNDS` = INCLUDE_BOUNDS, `OUTPUT_LINES` = OUTPUT_LINES, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:serviceareafrompoint", `INPUT` = INPUT, `STRATEGY` = STRATEGY, `DIRECTION_FIELD` = DIRECTION_FIELD, `VALUE_FORWARD` = VALUE_FORWARD, `VALUE_BACKWARD` = VALUE_BACKWARD, `VALUE_BOTH` = VALUE_BOTH, `DEFAULT_DIRECTION` = DEFAULT_DIRECTION, `SPEED_FIELD` = SPEED_FIELD, `DEFAULT_SPEED` = DEFAULT_SPEED, `TOLERANCE` = TOLERANCE, `START_POINT` = START_POINT, `TRAVEL_COST2` = TRAVEL_COST2, `INCLUDE_BOUNDS` = INCLUDE_BOUNDS, `OUTPUT_LINES` = OUTPUT_LINES, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
