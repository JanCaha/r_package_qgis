##' QGIS Algorithm provided by GRASS v.lidar.growing (grass7:v.lidar.growing). Building contour determination and Region Growing algorithm for determining the building inside
##'
##' @title QGIS algorithm - v.lidar.growing
##'
##' @param input `source` - Input vector (v.lidar.edgedetection output). Path to a vector layer.
##' @param first `source` - First pulse vector layer. Path to a vector layer.
##' @param tj `number` - Threshold for cell object frequency in region growing. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param td `number` - Threshold for double pulse in region growing. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param output `vectorDestination` - Buildings. Path for new vector layer.
