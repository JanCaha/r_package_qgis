##' QGIS Algorithm provided by GRASS r.walk.rast (grass7:r.walk.rast). r.walk.rast - Creates a raster map showing the anisotropic cumulative cost of moving between different geographic locations on an input raster map whose cell category values represent cost from a raster.
##'
##' @title QGIS algorithm - r.walk.rast
##'
...
##' @param start_raster `raster` - Name of starting raster points map (all non-NULL cells are starting points). Path to a raster layer.
