##' QGIS Algorithm provided by GRASS v.rast.stats (grass:v.rast.stats). Calculates univariate statistics from a raster map based on vector polygons and uploads statistics to new attribute columns.
##'
##' @title QGIS algorithm - v.rast.stats
##'
##' @param map `source` - Name of vector polygon map. Path to a vector layer.
##' @param raster `raster` - Name of raster map to calculate statistics from. Path to a raster layer.
##' @param column_prefix `string` - Column prefix for new attribute columns. String value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param method `enum`  of `("number", "minimum", "maximum", "range", "average", "stddev", "variance", "coeff_var", "sum", "first_quartile", "median", "third_quartile", "percentile")` - The methods to use. Number of selected option, e.g. '1'. Comma separated list of options, e.g. '1,3'.
...
##' @param output `vectorDestination` - Rast stats. Path for new vector layer.
...
##' @details
##' ## Outputs description
##' * output - outputVector - Rast stats
##'
##'
##' @export
##' @md
...
grass_v_rast_stats <- function(map = qgisprocess:::qgis_default_value(), raster = qgisprocess:::qgis_default_value(), column_prefix = qgisprocess:::qgis_default_value(), method = qgisprocess:::qgis_default_value(), percentile = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_SNAP_TOLERANCE_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_MIN_AREA_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_OUTPUT_TYPE_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_VECTOR_DSCO = qgisprocess:::qgis_default_value(), GRASS_VECTOR_LCO = qgisprocess:::qgis_default_value(), GRASS_VECTOR_EXPORT_NOCAT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:v.rast.stats")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:v.rast.stats", `map` = map, `raster` = raster, `column_prefix` = column_prefix, `method` = method, `percentile` = percentile, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER, `GRASS_SNAP_TOLERANCE_PARAMETER` = GRASS_SNAP_TOLERANCE_PARAMETER, `GRASS_MIN_AREA_PARAMETER` = GRASS_MIN_AREA_PARAMETER, `GRASS_OUTPUT_TYPE_PARAMETER` = GRASS_OUTPUT_TYPE_PARAMETER, `GRASS_VECTOR_DSCO` = GRASS_VECTOR_DSCO, `GRASS_VECTOR_LCO` = GRASS_VECTOR_LCO, `GRASS_VECTOR_EXPORT_NOCAT` = GRASS_VECTOR_EXPORT_NOCAT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:v.rast.stats", `map` = map, `raster` = raster, `column_prefix` = column_prefix, `method` = method, `percentile` = percentile, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER, `GRASS_SNAP_TOLERANCE_PARAMETER` = GRASS_SNAP_TOLERANCE_PARAMETER, `GRASS_MIN_AREA_PARAMETER` = GRASS_MIN_AREA_PARAMETER, `GRASS_OUTPUT_TYPE_PARAMETER` = GRASS_OUTPUT_TYPE_PARAMETER, `GRASS_VECTOR_DSCO` = GRASS_VECTOR_DSCO, `GRASS_VECTOR_LCO` = GRASS_VECTOR_LCO, `GRASS_VECTOR_EXPORT_NOCAT` = GRASS_VECTOR_EXPORT_NOCAT,..., .quiet = .quiet)
      )
