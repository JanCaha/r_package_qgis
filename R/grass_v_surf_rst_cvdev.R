##' QGIS Algorithm provided by GRASS v.surf.rst.cvdev (grass:v.surf.rst.cvdev). v.surf.rst.cvdev - Performs surface interpolation from vector points map by splines.
##'
...
##' @param .c `boolean` - Perform cross-validation procedure without raster approximation (leave this option as True). 1 for true/yes. 0 for false/no. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression. Original algorithm parameter name: -c.
