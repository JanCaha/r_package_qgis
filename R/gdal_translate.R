...
gdal_translate <- function(INPUT = qgisprocess:::qgis_default_value(), TARGET_CRS = qgisprocess:::qgis_default_value(), NODATA = qgisprocess:::qgis_default_value(), COPY_SUBDATASETS = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), DATA_TYPE = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("gdal:translate")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:translate", `INPUT` = INPUT, `TARGET_CRS` = TARGET_CRS, `NODATA` = NODATA, `COPY_SUBDATASETS` = COPY_SUBDATASETS, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:translate", `INPUT` = INPUT, `TARGET_CRS` = TARGET_CRS, `NODATA` = NODATA, `COPY_SUBDATASETS` = COPY_SUBDATASETS, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `DATA_TYPE` = DATA_TYPE, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
