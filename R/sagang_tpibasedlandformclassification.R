...
sagang_tpibasedlandformclassification <- function(DEM = qgisprocess:::qgis_default_value(), LANDFORMS = qgisprocess:::qgis_default_value(), DW_WEIGHTING = qgisprocess:::qgis_default_value(), DW_IDW_POWER = qgisprocess:::qgis_default_value(), DW_BANDWIDTH = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:tpibasedlandformclassification")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:tpibasedlandformclassification", `DEM` = DEM, `LANDFORMS` = LANDFORMS, `DW_WEIGHTING` = DW_WEIGHTING, `DW_IDW_POWER` = DW_IDW_POWER, `DW_BANDWIDTH` = DW_BANDWIDTH,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:tpibasedlandformclassification", `DEM` = DEM, `LANDFORMS` = LANDFORMS, `DW_WEIGHTING` = DW_WEIGHTING, `DW_IDW_POWER` = DW_IDW_POWER, `DW_BANDWIDTH` = DW_BANDWIDTH,..., .quiet = .quiet)
      )
