##' QGIS Algorithm provided by GRASS r.out.png (grass:r.out.png). Export a GRASS raster map as a non-georeferenced PNG image
##'
...
grass_r_out_png <- function(input = qgisprocess:::qgis_default_value(), compression = qgisprocess:::qgis_default_value(), .t = qgisprocess:::qgis_default_value(), .w = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:r.out.png")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:r.out.png", `input` = input, `compression` = compression, `-t` = .t, `-w` = .w, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:r.out.png", `input` = input, `compression` = compression, `-t` = .t, `-w` = .w, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
