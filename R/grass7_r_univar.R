##' QGIS Algorithm provided by GRASS r.univar (grass7:r.univar). Calculates univariate statistics from the non-null cells of a raster map.
##'
...
grass7_r_univar <- function(map = qgisprocess:::qgis_default_value(), zones = qgisprocess:::qgis_default_value(), percentile = qgisprocess:::qgis_default_value(), separator = qgisprocess:::qgis_default_value(), .e = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {
  ...
  check_algorithm_necessities("grass7:r.univar")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass7:r.univar", `map` = map, `zones` = zones, `percentile` = percentile, `separator` = separator, `-e` = .e, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass7:r.univar", `map` = map, `zones` = zones, `percentile` = percentile, `separator` = separator, `-e` = .e, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
