...
##' @param threshold `number` - Threshold for connecting nodes to the network (in map unit). A numeric value.
##' @param arc_type `enum`  of `("line", "boundary")` - Arc type. Number of selected option, e.g. '1'. Comma separated list of options, e.g. '1,3'.
##' @param from_cats `string` - From Category values. String value.
##' @param from_where `string` - From WHERE conditions of SQL statement without 'where' keyword. String value.
...
##' @param to_cats `string` - To Category values. String value.
##' @param to_where `string` - To WHERE conditions of SQL statement without 'where' keyword. String value.
...
##' @param .l `boolean` - Write each output path as one line, not as original input segments. 1 for true/yes. 0 for false/no. Original algorithm parameter name: {name}.
