...
sagang_coordinatetransformationshapeslist <- function(CRS_METHOD = qgisprocess:::qgis_default_value(), CRS_PROJ4 = qgisprocess:::qgis_default_value(), CRS_FILE = qgisprocess:::qgis_default_value(), CRS_EPSG = qgisprocess:::qgis_default_value(), CRS_EPSG_AUTH = qgisprocess:::qgis_default_value(), SOURCE = qgisprocess:::qgis_default_value(), TARGET = qgisprocess:::qgis_default_value(), TRANSFORM_Z = qgisprocess:::qgis_default_value(), PARALLEL = qgisprocess:::qgis_default_value(), COPY = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:coordinatetransformationshapeslist")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:coordinatetransformationshapeslist", `CRS_METHOD` = CRS_METHOD, `CRS_PROJ4` = CRS_PROJ4, `CRS_FILE` = CRS_FILE, `CRS_EPSG` = CRS_EPSG, `CRS_EPSG_AUTH` = CRS_EPSG_AUTH, `SOURCE` = SOURCE, `TARGET` = TARGET, `TRANSFORM_Z` = TRANSFORM_Z, `PARALLEL` = PARALLEL, `COPY` = COPY,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:coordinatetransformationshapeslist", `CRS_METHOD` = CRS_METHOD, `CRS_PROJ4` = CRS_PROJ4, `CRS_FILE` = CRS_FILE, `CRS_EPSG` = CRS_EPSG, `CRS_EPSG_AUTH` = CRS_EPSG_AUTH, `SOURCE` = SOURCE, `TARGET` = TARGET, `TRANSFORM_Z` = TRANSFORM_Z, `PARALLEL` = PARALLEL, `COPY` = COPY,..., .quiet = .quiet)
      )
