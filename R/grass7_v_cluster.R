##' QGIS Algorithm provided by GRASS v.cluster (grass7:v.cluster). Performs cluster identification
##'
##' @title QGIS algorithm - v.cluster
##'
##' @param input `source` - Input layer. Path to a vector layer.
##' @param distance `number` - Maximum distance to neighbors. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param min `number` - Minimum number of points to create a cluster. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
...
##' @param .arg_2 `boolean` - Force 2D clustering. 1 for true/yes. 0 for false/no. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression. Original algorithm parameter name: -2.
