...
sagang_tcilow <- function(DISTANCE = qgisprocess:::qgis_default_value(), TWI = qgisprocess:::qgis_default_value(), TCILOW = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:tcilow")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:tcilow", `DISTANCE` = DISTANCE, `TWI` = TWI, `TCILOW` = TCILOW,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:tcilow", `DISTANCE` = DISTANCE, `TWI` = TWI, `TCILOW` = TCILOW,..., .quiet = .quiet)
      )
