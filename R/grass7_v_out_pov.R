##' QGIS Algorithm provided by GRASS v.out.pov (grass7:v.out.pov). Converts to POV-Ray format, GRASS x,y,z - POV-Ray x,z,y
##'
...
grass7_v_out_pov <- function(input = qgisprocess:::qgis_default_value(), type = qgisprocess:::qgis_default_value(), size = qgisprocess:::qgis_default_value(), zmod = qgisprocess:::qgis_default_value(), objmod = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_SNAP_TOLERANCE_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_MIN_AREA_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {
  ...
  check_algorithm_necessities("grass7:v.out.pov")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass7:v.out.pov", `input` = input, `type` = type, `size` = size, `zmod` = zmod, `objmod` = objmod, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_SNAP_TOLERANCE_PARAMETER` = GRASS_SNAP_TOLERANCE_PARAMETER, `GRASS_MIN_AREA_PARAMETER` = GRASS_MIN_AREA_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass7:v.out.pov", `input` = input, `type` = type, `size` = size, `zmod` = zmod, `objmod` = objmod, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_SNAP_TOLERANCE_PARAMETER` = GRASS_SNAP_TOLERANCE_PARAMETER, `GRASS_MIN_AREA_PARAMETER` = GRASS_MIN_AREA_PARAMETER,..., .quiet = .quiet)
      )
