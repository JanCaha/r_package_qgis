##' QGIS Algorithm provided by GRASS r.li.edgedensity.ascii (grass:r.li.edgedensity.ascii). r.li.edgedensity.ascii - Calculates edge density index on a raster map, using a 4 neighbour algorithm
##'
...
grass_r_li_edgedensity_ascii <- function(input = qgisprocess:::qgis_default_value(), config_txt = qgisprocess:::qgis_default_value(), config = qgisprocess:::qgis_default_value(), patch_type = qgisprocess:::qgis_default_value(), .b = qgisprocess:::qgis_default_value(), output_txt = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:r.li.edgedensity.ascii")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:r.li.edgedensity.ascii", `input` = input, `config_txt` = config_txt, `config` = config, `patch_type` = patch_type, `-b` = .b, `output_txt` = output_txt, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:r.li.edgedensity.ascii", `input` = input, `config_txt` = config_txt, `config` = config, `patch_type` = patch_type, `-b` = .b, `output_txt` = output_txt, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
