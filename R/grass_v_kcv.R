##' QGIS Algorithm provided by GRASS v.kcv (grass:v.kcv). Randomly partition points into test/train sets.
##'
##' @title QGIS algorithm - v.kcv
##'
##' @param map `source` - Input layer. Path to a vector layer.
##' @param npartitions `number` - Number of partitions. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param column `string` - Name for new column to which partition number is written. String value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
##' @param output `vectorDestination` - Partition. Path for new vector layer.
