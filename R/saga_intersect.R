...
saga_intersect <- function(A = qgisprocess:::qgis_default_value(), B = qgisprocess:::qgis_default_value(), SPLIT = qgisprocess:::qgis_default_value(), RESULT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("saga:intersect")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("saga:intersect", `A` = A, `B` = B, `SPLIT` = SPLIT, `RESULT` = RESULT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("saga:intersect", `A` = A, `B` = B, `SPLIT` = SPLIT, `RESULT` = RESULT,..., .quiet = .quiet)
      )
