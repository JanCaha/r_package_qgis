...
##' @param .k `boolean` - Preserve original cell values (By default original values are smoothed). 1 for true/yes. 0 for false/no. Original algorithm parameter name: {name}.
...
##' @param .m `boolean` - Interpret distance as map units, not number of cells (Do not select with geodetic coordinates). 1 for true/yes. 0 for false/no. Original algorithm parameter name: {name}.
