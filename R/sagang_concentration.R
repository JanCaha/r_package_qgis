...
sagang_concentration <- function(MASK = qgisprocess:::qgis_default_value(), GRAD = qgisprocess:::qgis_default_value(), CONC = qgisprocess:::qgis_default_value(), CONC_IN = qgisprocess:::qgis_default_value(), CONC_OUT = qgisprocess:::qgis_default_value(), CONC_E = qgisprocess:::qgis_default_value(), GRAD_MIN = qgisprocess:::qgis_default_value(), NEIGHBOURS = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:concentration")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:concentration", `MASK` = MASK, `GRAD` = GRAD, `CONC` = CONC, `CONC_IN` = CONC_IN, `CONC_OUT` = CONC_OUT, `CONC_E` = CONC_E, `GRAD_MIN` = GRAD_MIN, `NEIGHBOURS` = NEIGHBOURS,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:concentration", `MASK` = MASK, `GRAD` = GRAD, `CONC` = CONC, `CONC_IN` = CONC_IN, `CONC_OUT` = CONC_OUT, `CONC_E` = CONC_E, `GRAD_MIN` = GRAD_MIN, `NEIGHBOURS` = NEIGHBOURS,..., .quiet = .quiet)
      )
