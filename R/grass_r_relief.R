##' QGIS Algorithm provided by GRASS r.relief (grass:r.relief). Creates shaded relief from an elevation layer (DEM).
##'
...
grass_r_relief <- function(input = qgisprocess:::qgis_default_value(), altitude = qgisprocess:::qgis_default_value(), azimuth = qgisprocess:::qgis_default_value(), zscale = qgisprocess:::qgis_default_value(), scale = qgisprocess:::qgis_default_value(), units = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_RASTER_FORMAT_OPT = qgisprocess:::qgis_default_value(), GRASS_RASTER_FORMAT_META = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:r.relief")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:r.relief", `input` = input, `altitude` = altitude, `azimuth` = azimuth, `zscale` = zscale, `scale` = scale, `units` = units, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER, `GRASS_RASTER_FORMAT_OPT` = GRASS_RASTER_FORMAT_OPT, `GRASS_RASTER_FORMAT_META` = GRASS_RASTER_FORMAT_META,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:r.relief", `input` = input, `altitude` = altitude, `azimuth` = azimuth, `zscale` = zscale, `scale` = scale, `units` = units, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER, `GRASS_RASTER_FORMAT_OPT` = GRASS_RASTER_FORMAT_OPT, `GRASS_RASTER_FORMAT_META` = GRASS_RASTER_FORMAT_META,..., .quiet = .quiet)
      )
