##' QGIS Algorithm provided by QGIS (native c++) Set M value from raster (native:setmfromraster). Sets the M value for vertices to values sampled from a raster layer. This algorithm sets the M value for every vertex in the feature geometry to a value sampled from a band within a raster layer.  The raster values can optionally be scaled by a preset amount and an offset can be algebraically added.
##'
...
qgis_setmfromraster <- function(INPUT = qgisprocess:::qgis_default_value(), RASTER = qgisprocess:::qgis_default_value(), BAND = qgisprocess:::qgis_default_value(), NODATA = qgisprocess:::qgis_default_value(), SCALE = qgisprocess:::qgis_default_value(), OFFSET = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:setmfromraster")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:setmfromraster", `INPUT` = INPUT, `RASTER` = RASTER, `BAND` = BAND, `NODATA` = NODATA, `SCALE` = SCALE, `OFFSET` = OFFSET, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:setmfromraster", `INPUT` = INPUT, `RASTER` = RASTER, `BAND` = BAND, `NODATA` = NODATA, `SCALE` = SCALE, `OFFSET` = OFFSET, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
