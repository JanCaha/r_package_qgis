##' QGIS Algorithm provided by GRASS r.latlong (grass:r.latlong). Creates a latitude/longitude raster map.
##'
##' @title QGIS algorithm - r.latlong
##'
##' @param input `raster` - Name of input raster map. Path to a raster layer.
##' @param .l `boolean` - Longitude output. 1 for true/yes. 0 for false/no. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression. Original algorithm parameter name: -l.
