##' QGIS Algorithm provided by GRASS r.tile (grass:r.tile). Splits a raster map into tiles
##'
...
grass_r_tile <- function(input = qgisprocess:::qgis_default_value(), width = qgisprocess:::qgis_default_value(), height = qgisprocess:::qgis_default_value(), overlap = qgisprocess:::qgis_default_value(), output = qgisprocess:::qgis_default_value(), GRASS_REGION_PARAMETER = qgisprocess:::qgis_default_value(), GRASS_REGION_CELLSIZE_PARAMETER = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("grass:r.tile")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("grass:r.tile", `input` = input, `width` = width, `height` = height, `overlap` = overlap, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("grass:r.tile", `input` = input, `width` = width, `height` = height, `overlap` = overlap, `output` = output, `GRASS_REGION_PARAMETER` = GRASS_REGION_PARAMETER, `GRASS_REGION_CELLSIZE_PARAMETER` = GRASS_REGION_CELLSIZE_PARAMETER,..., .quiet = .quiet)
      )
