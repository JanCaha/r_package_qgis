##' QGIS Algorithm provided by GRASS r.param.scale (grass7:r.param.scale). Extracts terrain parameters from a DEM.
##'
...
##' @param size `number` - Size of processing window (odd number only, max: 69). A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
