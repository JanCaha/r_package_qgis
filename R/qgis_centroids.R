##' QGIS Algorithm provided by QGIS (native c++) Centroids (native:centroids). Creates a new point layer with points representing the centroid of the geometries in an input layer. This algorithm creates a new point layer with points representing the centroid of the geometries in an input layer.  The attributes associated to each point in the output layer are the same ones associated to the original features.  ---------------- Notes ----------------   - This algorithm may drop existing primary keys or FID values and regenerate them in output layers, depending on the input parameters.
##'
