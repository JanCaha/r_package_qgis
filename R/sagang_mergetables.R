...
##'
##' @title QGIS algorithm - Merge tables
##'
##' @param INPUT `multilayer` - Tables. .
##' @param MERGED `vectorDestination` - Merged Table. Path for new vector layer.
...
##' @details
##' ## Outputs description
##' * MERGED - outputVector - Merged Table
##'
##'
##' @export
##' @md
...
sagang_mergetables <- function(INPUT = qgisprocess:::qgis_default_value(), MERGED = qgisprocess:::qgis_default_value(), SRCINFO = qgisprocess:::qgis_default_value(), MATCH = qgisprocess:::qgis_default_value(), DELETE = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("sagang:mergetables")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("sagang:mergetables", `INPUT` = INPUT, `MERGED` = MERGED, `SRCINFO` = SRCINFO, `MATCH` = MATCH, `DELETE` = DELETE,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("sagang:mergetables", `INPUT` = INPUT, `MERGED` = MERGED, `SRCINFO` = SRCINFO, `MATCH` = MATCH, `DELETE` = DELETE,..., .quiet = .quiet)
      )
