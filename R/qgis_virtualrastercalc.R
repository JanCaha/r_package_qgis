...
qgis_virtualrastercalc <- function(LAYERS = qgisprocess:::qgis_default_value(), EXPRESSION = qgisprocess:::qgis_default_value(), EXTENT = qgisprocess:::qgis_default_value(), CELL_SIZE = qgisprocess:::qgis_default_value(), CRS = qgisprocess:::qgis_default_value(), LAYER_NAME = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("native:virtualrastercalc")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("native:virtualrastercalc", `LAYERS` = LAYERS, `EXPRESSION` = EXPRESSION, `EXTENT` = EXTENT, `CELL_SIZE` = CELL_SIZE, `CRS` = CRS, `LAYER_NAME` = LAYER_NAME,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("native:virtualrastercalc", `LAYERS` = LAYERS, `EXPRESSION` = EXPRESSION, `EXTENT` = EXTENT, `CELL_SIZE` = CELL_SIZE, `CRS` = CRS, `LAYER_NAME` = LAYER_NAME,..., .quiet = .quiet)
      )
