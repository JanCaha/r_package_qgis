##' QGIS Algorithm provided by GRASS r.terraflow (grass:r.terraflow). Flow computation for massive grids.
##'
##' @title QGIS algorithm - r.terraflow
##'
##' @param elevation `raster` - Name of elevation raster map. Path to a raster layer.
##' @param .s `boolean` - SFD (D8) flow (default is MFD). 1 for true/yes. 0 for false/no. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression. Original algorithm parameter name: -s.
##' @param d8cut `number` - Routing using SFD (D8) direction. A numeric value. field:FIELD_NAME to use a data defined value taken from the FIELD_NAME field. expression:SOME EXPRESSION to use a data defined value calculated using a custom QGIS expression.
