...
gdal_aspect <- function(INPUT = qgisprocess:::qgis_default_value(), BAND = qgisprocess:::qgis_default_value(), TRIG_ANGLE = qgisprocess:::qgis_default_value(), ZERO_FLAT = qgisprocess:::qgis_default_value(), COMPUTE_EDGES = qgisprocess:::qgis_default_value(), ZEVENBERGEN = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {
  ...
  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:aspect", `INPUT` = INPUT, `BAND` = BAND, `TRIG_ANGLE` = TRIG_ANGLE, `ZERO_FLAT` = ZERO_FLAT, `COMPUTE_EDGES` = COMPUTE_EDGES, `ZEVENBERGEN` = ZEVENBERGEN, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:aspect", `INPUT` = INPUT, `BAND` = BAND, `TRIG_ANGLE` = TRIG_ANGLE, `ZERO_FLAT` = ZERO_FLAT, `COMPUTE_EDGES` = COMPUTE_EDGES, `ZEVENBERGEN` = ZEVENBERGEN, `CREATION_OPTIONS` = CREATION_OPTIONS, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
