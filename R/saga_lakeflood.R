...
saga_lakeflood <- function(ELEV = qgisprocess::qgis_default_value(), SEEDS = qgisprocess::qgis_default_value(), OUTDEPTH = qgisprocess::qgis_default_value(), OUTLEVEL = qgisprocess::qgis_default_value(),..., .complete_output = TRUE) {

  check_algorithm_necessities("saga:lakeflood")

  output <- qgisprocess::qgis_run_algorithm("saga:lakeflood", `ELEV` = ELEV, `SEEDS` = SEEDS, `OUTDEPTH` = OUTDEPTH, `OUTLEVEL` = OUTLEVEL,...)
