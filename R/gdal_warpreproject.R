...
##' @param RESAMPLING `enum`  of `("Nearest Neighbour", "Bilinear (2x2 Kernel)", "Cubic (4x4 Kernel)", "Cubic B-Spline (4x4 Kernel)", "Lanczos (6x6 Kernel)", "Average", "Mode", "Maximum", "Minimum", "Median", "First Quartile (Q1)", "Third Quartile (Q3)")` - Resampling method to use. Number of selected option, e.g. '1'. Comma separated list of options, e.g. '1,3'.
...
gdal_warpreproject <- function(INPUT = qgisprocess:::qgis_default_value(), SOURCE_CRS = qgisprocess:::qgis_default_value(), TARGET_CRS = qgisprocess:::qgis_default_value(), RESAMPLING = qgisprocess:::qgis_default_value(), NODATA = qgisprocess:::qgis_default_value(), TARGET_RESOLUTION = qgisprocess:::qgis_default_value(), CREATION_OPTIONS = qgisprocess:::qgis_default_value(), DATA_TYPE = qgisprocess:::qgis_default_value(), TARGET_EXTENT = qgisprocess:::qgis_default_value(), TARGET_EXTENT_CRS = qgisprocess:::qgis_default_value(), MULTITHREADING = qgisprocess:::qgis_default_value(), EXTRA = qgisprocess:::qgis_default_value(), OUTPUT = qgisprocess:::qgis_default_value(),..., .complete_output = .complete_output_option(), .quiet = .quiet_option(), .messages = .message_option()) {

  check_algorithm_necessities("gdal:warpreproject")

  if (.messages){
    output <- qgisprocess::qgis_run_algorithm("gdal:warpreproject", `INPUT` = INPUT, `SOURCE_CRS` = SOURCE_CRS, `TARGET_CRS` = TARGET_CRS, `RESAMPLING` = RESAMPLING, `NODATA` = NODATA, `TARGET_RESOLUTION` = TARGET_RESOLUTION, `CREATION_OPTIONS` = CREATION_OPTIONS, `DATA_TYPE` = DATA_TYPE, `TARGET_EXTENT` = TARGET_EXTENT, `TARGET_EXTENT_CRS` = TARGET_EXTENT_CRS, `MULTITHREADING` = MULTITHREADING, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
  } else {
    suppressMessages(
      output <- qgisprocess::qgis_run_algorithm("gdal:warpreproject", `INPUT` = INPUT, `SOURCE_CRS` = SOURCE_CRS, `TARGET_CRS` = TARGET_CRS, `RESAMPLING` = RESAMPLING, `NODATA` = NODATA, `TARGET_RESOLUTION` = TARGET_RESOLUTION, `CREATION_OPTIONS` = CREATION_OPTIONS, `DATA_TYPE` = DATA_TYPE, `TARGET_EXTENT` = TARGET_EXTENT, `TARGET_EXTENT_CRS` = TARGET_EXTENT_CRS, `MULTITHREADING` = MULTITHREADING, `EXTRA` = EXTRA, `OUTPUT` = OUTPUT,..., .quiet = .quiet)
      )
